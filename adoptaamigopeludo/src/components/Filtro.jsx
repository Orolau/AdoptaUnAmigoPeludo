...
const Filtro = ({ funcionMostrarMascotas, listaMascotas }) => {
    ...
    const aplicarFiltro = (event) => {
        ...
        const mascotasFiltradas = listaMascotas.filter(mascota => {
            ...
            const edadMascota = convertirEdad(mascota.edad.toLowerCase());
            const cumpleEdad = filtroEdad ? edadMascota <= parseInt(filtroEdad) : true;
    ...
    const convertirEdad = (edadTexto) => {
        ...
        if (edadTexto.includes('año')) {
            const anhos = parseInt(edadTexto.split(' ')[0]);
